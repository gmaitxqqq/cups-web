...
| **发票 N 合 1 排版** | 发票合并支持四种版面：**纵向 2 合 1 / 横向 2 合 1 / 纵向 4 合 1 / 横向 4 合 1**，可在「发票打印」面板直接切换 |
...
4. 左上角版本号应为当前构建的 SHA（如 `invoice-nup`），确认部署版本无误。

### 验证 4：发票 N 合 1 排版
1. 进入「发票打印」模式，上传多张发票；
2. 在发票列表下方的「排版」里任选：**纵向 2 合 1 / 横向 2 合 1 / 纵向 4 合 1 / 横向 4 合 1**；
3. 点击「合并预览」，预览会实时反映所选版面（横向版面为横版纸张，4 合 1 每页塞下 4 张）；
