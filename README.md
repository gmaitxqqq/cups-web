# cups-web 定制版（免登录 · 发票增强 · 图片打印排版）

本仓库是基于 [hanxi/cups-web](https://github.com/hanxi/cups-web) 的定制 fork，面向家庭 / 内网打印机场景，在原版基础上做了若干实用增强。所有改动已编译进同一个镜像，开箱即用。

---

## 一、功能特性（相比原版）

| 特性 | 说明 |
|---|---|
| **免登录** | 设置 `AUTH_DISABLED=true` 后直接打开主界面，无需账号密码（仅限内网） |
| **发票金额修复** | 打印 / 合成前自动用 Ghostscript 嵌入缺失字体，解决发票 PDF 金额空白问题 |
| **图片手动缩放** | 图片支持「自动适应」或「手动缩放 1–100%」，进度条 / 输入框均可调 |
| **水平对齐** | 图片支持 **居中 / 居左 / 居右** |
| **垂直对齐** | 图片支持 **居中 / 靠上 / 靠下**（纵向图片也能选，不再只能居中） |
| **发票 N 合 1 版面** | 发票合并支持四种版面：**纵向 2 合 1 / 横向 2 合 1 / 纵向 4 合 1 / 横向 4 合 1**，可在「发票打印」面板直接切换 |
| **发票边距可调** | 发票合并支持「边距」滑块（**0–20mm，默认 5mm**），自由控制每张发票四周留白，实时预览 |
| **预览逐页翻页** | 发票 / 合成预览改为**逐页返回**，带「‹ 1 / N ›」翻页条，不再被误认为单页长图 |
| **预览方向自动跟随** | 发票模式预览纸张方向**随所选版面自动切换**（纵向版面 = 竖版、横向版面 = 横版），手动切换在发票模式自动禁用并标注「自动」 |
| **标准模式多页预览翻页** | 标准打印 / 图片 / 文档上传后，预览同样**逐页返回并带「‹ 1 / N ›」翻页条**（与发票模式一致），不再只显示首页；加密 / 特殊结构 PDF 也按真实页数渲染 |
| **服务器侧预览** | 预览由后端渲染成 PNG 返回，绕过 pdf.js，所有 PDF / 图片预览稳定 |
| **版本号显示** | 左上角显示构建版本号，**tag 优先**（如 `v1.1.4`；领先 tag 若干提交时为 `v1.1.4-3-gabcdef`；无 tag 才回退短 SHA），方便核对当前部署对应哪个 GitHub 版本 |

> 图片排版相关选项（缩放方式、水平对齐、垂直对齐）**仅在选择图片文件时出现**，普通 PDF / Office 文档走原有排版逻辑，互不影响。
> 「边距 / 预览方向自动跟随」仅在「发票打印」模式生效；而「**逐页预览翻页**」在**所有模式通用**（发票 / 标准打印 / 图片 / 文档上传后均可翻页查看每一页）。

---

## 二、快速开始（Docker 部署）

把下面这份 `docker-compose.yml` 放到一个目录，执行 `docker compose up -d` 即可。

关键两点：**换镜像地址** + **加 `AUTH_DISABLED=true`**。

```yaml
services:
  cups:
    # 你自己构建的镜像（也可改回 hanxi/cups-web:latest 还原原版）
    image: ghcr.io/gmaitxqqq/cups-web:latest
    container_name: cups
    user: root
    security_opt:
      - apparmor:unconfined
    environment:
      - CUPSADMIN=${CUPSADMIN:-print}
      - CUPSPASSWORD=${CUPSPASSWORD:-print}
      - TZ=${TZ:-Asia/Shanghai}
      - AUTH_DISABLED=true          # ← 免登录直接打开
    ports:
      - "631:631"
      - "1180:8080"
    volumes:
      - ./.etc:/etc/cups
      - ./.data:/data
      - ./.uploads:/uploads
      - ./.drivers:/opt/cups-drivers/data
      - /dev/bus/usb:/dev/bus/usb
      - /run/udev:/run/udev:ro
    device_cgroup_rules:
      - 'c 189:* rmw'
    restart: unless-stopped
```

打开 `http://<你的机器IP>:1180` —— 应该**直接进入主界面，没有登录框**。

> **镜像大小说明**：完整镜像约 **665MB**（未压缩）。你在 GitHub 仓库页看到的「200 KB」是**源码仓库**体积，不是镜像——镜像拉下来几百 MB 属正常。
> 国内访问 `ghcr.io` 偶尔会被限速，若 `docker compose pull` 长时间卡住，可参考第七节「本地交叉编译 + 局域网中转部署」。

如果还想保留原版账号密码登录，删掉（或注释掉）`AUTH_DISABLED=true` 即可，行为与原版一致。

---

## 三、图片打印排版（本版重点增强）

上传一张图片（或勾选「合并多张图片」）后，「打印参数」面板会出现一个 **「图片排版」** 区块，包含三组控制：

### 1. 缩放方式
- **自动适应（默认）**：按图片原始比例等比缩放到页边距内最大可放区域，不会变形、不会溢出页面。
- **手动缩放**：在「自动适应」尺寸基础上再乘以 **1%–100%** 的百分比（100% = 与自动适应同等大小，可向下缩小到 1%）。配合右侧的数值输入框或滑块实时调整，预览即时刷新。

> 之前「放一张图就默认铺满全屏、缩放没用」的问题已修复：现在默认是保持比例的「自动适应」，手动缩放让你精确控制最终大小。

### 2. 水平对齐
| 选项 | 效果 |
|---|---|
| 居中 | 图片在可用宽度内水平居中（默认） |
| 居左 | 图片贴左边距 |
| 居右 | 图片贴右边距 |

### 3. 垂直对齐
| 选项 | 效果 |
|---|---|
| 居中 | 图片在可用高度内垂直居中（默认） |
| 靠上 | 图片贴上边距 |
| 靠下 | 图片贴下边距 |

> 纵向图片（如长截图、证件照）现在可以选「靠上 / 靠下」，不再被强制垂直居中。

三组选项任意组合，例如「手动缩放 50% + 居左 + 靠上」可把小图固定到页面左上角。

---

## 四、发票金额修复

**根因**：发票 PDF 里金额本身存在、颜色也是黑色，但金额用的 `STSong-Light` 字体**没有嵌入** PDF 文件。
原版为了性能改成「PDF 原样直接丢给 CUPS 打印」，容器里找不到这个字体 → 金额渲染成空白。
（普通打印、发票 N 合 1 合成两条路径都中招。）

**修复**：让「普通打印」和「发票 N 合 1 合成」两条路径在发送前，都先调用项目已有的 `normalizePDF()`
（Ghostscript `-dEmbedAllFonts=true`），把字体强制嵌入后再打印。

> Docker 镜像在构建时已装好 **Ghostscript + CJK 字体 + Windows 中文字体（宋体/黑体/楷体/仿宋）+ cidfmap 映射**，
> 所以未嵌入的 `STSong-Light` 会被正确替换并嵌入真字体，金额即可正常打印。
> **你本地不需要额外装字体**——只要用本仓库构建出来的镜像即可。

---

## 五、发票打印增强（本版重点）

进入「发票打印」模式后，上传多张发票即可使用以下增强能力。

### 5.1 N 合 1 版面

在发票列表下方的「排版」里任选四种版面之一：

| 版面 | 纸张方向 | 每页张数 |
|---|---|---|
| 纵向 2 合 1 | 竖版（Portrait） | 2 张 |
| 横向 2 合 1 | 横版（Landscape） | 2 张 |
| 纵向 4 合 1 | 竖版（Portrait） | 4 张 |
| 横向 4 合 1 | 横版（Landscape） | 4 张 |

**分页规则**：每页固定放 N 张（2 合 1 → 每页 2 张，4 合 1 → 每页 4 张），超出自动开新页。
例如 4 张发票选「纵向 2 合 1」→ 2 页（每页 2 张）；6 张 → 3 页；4 张选「纵向 4 合 1」→ 1 页。

### 5.2 边距可调

发票合并支持「边距」滑块，范围 **0–20mm**，默认 **5mm**（原版为硬编码 10mm）。
拖动滑块即实时刷新预览，用来控制每张发票在所属插槽内的四周留白大小——
边距越小，发票在半页 / 四分之一页插槽里越「铺满」；边距越大越留白。

### 5.3 预览逐页翻页

发票 / 合成的预览改为**后端逐页渲染、前端逐页展示**，预览区下方出现翻页条：

```
‹   1 / 2   ›
```

- 左 / 右箭头在页数间切换，当前页号居中显示；
- 彻底解决了早期「多页被拼成一张长图、看起来像全挤在一页」的视觉误解；
- 图片转 PDF、Ghostscript 标准化、PDF 自动栅格化等预览路径同样走逐页逻辑。

### 5.4 预览方向自动跟随版面

发票模式下，预览纸张方向**不再需要手动切换**，而是由所选版面自动决定：

- 选任意「纵向」版面 → 预览自动切竖版；
- 选任意「横向」版面 → 预览自动切横版；

手动「纵向 / 横向」切换按钮在发票模式下**自动禁用**，并显示「自动」字样，切换版面时方向随之联动刷新。

> 普通图片 / 文档打印仍保留手动切换纸张方向的能力，不受影响。

### 5.5 标准打印模式多页预览翻页

标准打印（普通 PDF / 图片 / 文档）上传后，预览同样会**逐页渲染并返回全部页面**，预览区下方出现与发票模式一致的翻页条：

```
‹   1 / N   ›
```

- 点击左右箭头即可翻看每一页，不再像早期版本那样「只显示首页、且不能翻页」；
- 预览上方会提示「多页预览 · 点击左右箭头可翻页查看每一页」，引导翻页；
- **加密 / 特殊结构 PDF 也正确按真实页数显示**：早期版本对带空口令加密的 PDF（如「二年级语文入学测试卷.pdf」实有 5 页）误判为 1 页，已修复——预览与打印光栅化的页数改由 Ghostscript 实际渲染产出的图片数决定，不再依赖不可靠的 unidoc 计数。

---

## 六、怎么验证改动都生效

### 验证 1：免登录
1. 浏览器清掉该站点的 cookie（或用无痕窗口）；
2. 打开 `http://<IP>:1180`；
3. 应直接进入打印主界面，没有登录页、不要求输入账号密码。

### 验证 2：发票金额能打印
1. 上传那张发票 PDF，普通打印或「发票打印（N 合 1 合成）」；
2. 看容器日志（`docker compose logs cups`）：
   - 出现 `pdf normalized (method=ghostscript) before printing to embed fonts` 代表字体已嵌入修复生效；
   - 打印出来的纸上金额数字应正常显示。

### 验证 3：图片缩放 + 对齐
1. 上传一张图片，确认出现「图片排版」区块；
2. 把「缩放方式」切到「手动缩放」，拖动滑块，预览应实时变大/变小；
3. 切换「水平对齐」（居中/居左/居右）与「垂直对齐」（居中/靠上/靠下），预览中图片落点应随之改变；
4. 左上角版本号应为当前构建的 SHA，确认部署版本无误。

### 验证 4：发票 N 合 1 版面 + 分页
1. 进入「发票打印」模式，上传多张发票；
2. 在发票列表下方的「排版」里任选版面（纵向/横向 × 2/4）；
3. 点击「合并预览」，预览应反映所选版面（横向版面为横版纸张，4 合 1 每页塞下 4 张）；
4. 切换排版时预览自动刷新，无需重新上传；
5. 上传 4 张选「2 合 1」应得到 2 页、选「4 合 1」应得到 1 页，确认分页正确。

### 验证 5：发票边距可调
1. 在「发票打印」模式拖动「边距」滑块（0–20mm）；
2. 预览中每张发票的四周留白应实时变化，默认 5mm。

### 验证 6：预览逐页翻页
1. 合成 / 发票预览出现后，预览区下方应有「‹ 1 / N ›」翻页条；
2. 点左右箭头可在各页间切换，当前页号正确。

### 验证 7：预览方向自动跟随
1. 发票模式下分别选「纵向 2 合 1」与「横向 2 合 1」；
2. 预览纸张方向应自动在竖版 / 横版间切换；
3. 手动「纵向 / 横向」按钮在发票模式下为禁用状态并显示「自动」。

### 验证 8：标准模式多页预览翻页（含加密 PDF）
1. 标准模式下上传一份多页 PDF（或用「二年级语文入学测试卷.pdf」这种**带空口令加密**的 5 页 PDF）；
2. 预览应**逐页渲染并返回全部页数**，预览区下方出现「‹ 1 / 5 ›」翻页条，上方有翻页提示；
3. 点左右箭头可翻看每一页，确认不再只显示首页；
4. 点「打印」应能正确输出全部页数（不再只打 1 页）。

---

## 七、构建与部署

### 方式 A：用 GitHub Actions 自动构建（推荐，已配好）
代码推到 `master` 分支后，`.github/workflows/build.yml` 会自动：
1. 构建镜像（单平台 `linux/amd64`）；
2. 登录 `ghcr.io` 并推送为 `ghcr.io/gmaitxqqq/cups-web:latest` 以及 `sha-<短哈希>` 标签。

查看构建进度：`https://github.com/gmaitxqqq/cups-web/actions`

> 注：仅 `push` 到 `master` 触发构建；打 `vX.Y.Z` 语义化 tag **不会**触发镜像重建（见第九节发版说明）。

### 方式 B：本机手动构建
前置要求：
- **Go 1.26**（由 `go.mod` 指定，`go.dev/dl` 下载）
- **Docker**（构建过程会 `docker build` 出镜像；镜像内会自动装 Ghostscript + 字体，**本机不必单独装 Ghostscript**）

```bash
# 在本仓库根目录
docker build -t cups-web:local .

# 跑起来（免登录）
AUTH_DISABLED=true docker run --rm -p 1180:8080 -p 631:631 \
  -e AUTH_DISABLED=true cups-web:local
```

### 方式 C：本地交叉编译 + 局域网中转部署（ghcr.io 被限速时）
当生产机从 `ghcr.io` 拉取镜像极慢（实测可能只有数 KB/s）时，可绕过镜像仓库，直接把编译好的二进制传过去：

```bash
# 1) 前端构建（node 已装好依赖时）
cd frontend && npm run build && cd ..

# 2) 交叉编译静态 Linux 二进制（前端经 go:embed 打进二进制，无需容器）
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
  go build -ldflags="-s -w -X main.Version=$(git rev-parse --short HEAD)" \
  -o cups-web-linux ./cmd/server

# 3) 通过 scp 传到生产机，再打一个只替换 /cups-web 的极小镜像
scp cups-web-linux root@<生产机IP>:/tmp/
ssh root@<生产机IP> 'docker cp /tmp/cups-web <容器名>:/cups-web && docker restart <容器名>'
```

> 该方式依赖 `CGO_ENABLED=0`（Dockerfile 中已设置），生成的纯静态二进制不依赖宿主机 libcups，
> 直接在现有容器里替换即可生效，**完全不碰外网镜像仓库**。

### 还原成官方原版
把 `docker-compose.yml` 里的镜像改回 `hanxi/cups-web:latest`，并删掉（或注释掉）`AUTH_DISABLED=true`，再 `docker compose up -d` 即可。

---

## 八、环境变量一览

| 变量 | 默认值 | 说明 |
|---|---|---|
| `AUTH_DISABLED` | 未设置（需登录） | `true` 时免登录直接打开主界面（仅限内网） |
| `CUPSADMIN` | `print` | CUPS 管理员账号 |
| `CUPSPASSWORD` | `print` | CUPS 管理员密码 |
| `TZ` | `Asia/Shanghai` | 容器时区 |
| `COOKIE_SECURE` | `false` | HTTPS 部署时设 `true`，让 cookie 带 Secure 属性 |

> ⚠️ **安全提示**：`AUTH_DISABLED=true` 开启后 `/api/admin`（用户管理 / 驱动管理 / 系统设置）也全部开放。
> 请**只在可信内网**使用，不要把这个端口暴露到公网。

---

## 九、更新日志与发版方式

### 更新日志（摘要）

完整记录见 [CHANGELOG.md](CHANGELOG.md)。

| 版本 | 日期 | 要点 |
|---|---|---|
| **v1.1.3** | 2026-09-20 | 修复横向打印纸张方向错误（横向 N 合 1 打成纵向、内容被裁切）：提交前用 Ghostscript 把横向作业旋转归一化为纵向 A4 再按 portrait 提交，预览仍显横向版面 |
| **v1.1.2** | 2026-08-21 | 修复 docx / 手动转换 PDF 预览失败（"PDF 预览加载失败"）：PDF 专属分支走 Ghostscript 光栅化预览，彻底绕开 pdf.js |
| **v1.1.1** | 2026-08-20 | 标准模式多页预览翻页（与发票一致，带「‹ 1 / N ›」翻页条）；修复加密 / 特殊结构 PDF 标准打印 / 预览只出 1 页（页数改由 Ghostscript 实际渲染决定） |
| **v1.1.0** | 2026-08-15 | 发票边距可调（0–20mm）、预览逐页翻页、预览方向自动跟随版面；N 合 1 版面（纵/横 × 2/4）与正确分页 |
| v1.0.0 | — | 定制 fork 首版：免登录、发票金额修复（Ghostscript 嵌入字体）、图片手动缩放 + 水平/垂直对齐、服务器侧预览、版本号显示 |

### 发版流程（如何打一个版本）

本仓用语义化版本 `vX.Y.Z` 打 tag，配套脚本 `bump-version.sh`：

```bash
# 在干净工作区执行，按改动类型升级（默认 patch）
./bump-version.sh patch     # v1.1.0 -> v1.1.1
./bump-version.sh minor     # v1.1.0 -> v1.2.0
./bump-version.sh major     # v1.1.0 -> v2.0.0
```

脚本会：
1. 取最新 `v*` tag 并按语义化版本号解析（严格只认 `vMAJOR.MINOR.PATCH`，忽略 `cups-driver` 等非版本 tag）；
2. 校验工作区干净后创建新 tag 并推送到 `origin`；
3. 由你在 GitHub 上基于该 tag 创建 Release，把 [CHANGELOG.md](CHANGELOG.md) 对应段落作为 Release Notes 粘贴发布。

> 提醒：推 `master` 才会触发镜像重建（`.github/workflows/build.yml`）；**打版本 tag 不会自动重建镜像**，
> 因此发版时请先确保 `master` 已包含本次改动并已完成一次镜像构建，再打 tag + 发 Release。
