...
func printHandler(w http.ResponseWriter, r *http.Request) {
	...
	case fileKindOFD:
		...
		printMime = "application/pdf"
	case fileKindImage:
		outPath, cleanup, err := convertImageToPDF(storedAbs, orientation, paperSize, 0, "center", "center")
		if err != nil {
			_ = os.Remove(storedAbs)
			writeJSONError(w, http.StatusBadRequest, "conversion failed")
			return
	...
	if saveHistory {
		err = appStore.WithTx(r.Context(), false, func(tx *sql.Tx) error {
			...
			if sess.UserID > 0 {
				...
				// 免登录模式：没有真实用户，用内置 admin 记录打印历史。
				user, err = store.GetUserByUsername(r.Context(), tx, "admin")
			}
			if err != nil {
				return err
			}
