...
//   - 页数直接由 GS 实际产出的 p*.png 文件数决定（glob 收集），
//     **不依赖** unidoc 的 countPDFPages —— 加密 / 特殊结构 PDF 会被它误判成 1 页，
//     从而导致预览只显示首页。GS 渲染出几张图就有几页，最可靠。
func renderPDFToPreviewPages(ctx context.Context, pdfPath string) ([]string, func(), error) {
	...
	// 收集 GS 实际产出的各页 PNG（命名 p1.png / p2.png ...），按页码数字升序排列。
	// 这是真实页数：GS 渲染出几张图就有几页，不受 unidoc 误判影响。
	matches, _ := filepath.Glob(filepath.Join(tmpDir, "p*.png"))
	reNum := regexp.MustCompile(`(\d+)\.png$`)
